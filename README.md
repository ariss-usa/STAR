...
For XRP robots, STAR now supports both Bluetooth and local WiFi pairing. Put the controller machine and XRP on the same WiFi network, then select a discovered `XRP_WIFI:<ip>` entry in the local robot list and pair. Bluetooth XRP pairing continues to work as before.
