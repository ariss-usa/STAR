...
For XRP robots, STAR now supports both Bluetooth and local WiFi pairing. WiFi entries appear in the local robot dropdown as `XRP_WIFI:<ip>` (discovered from your local ARP table). Select one to pair over WiFi; Bluetooth XRP pairing continues to work as before. If discovery misses your robot, set `XRP_WIFI_IP` before launching STAR to add a manual candidate.
