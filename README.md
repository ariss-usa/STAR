...
For XRP robots, STAR now supports both Bluetooth and local WiFi pairing. Put the controller machine and XRP on the same WiFi network, then either select a discovered `XRP_WIFI:<ip>` entry or type the XRP IP directly in the local robot field and pair. Bluetooth XRP pairing continues to work as before.
